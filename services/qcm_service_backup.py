...
JSON:"""
            ...
            try:
                ...
                return {
                    "success": False,
                    "error": f"Format JSON invalide: {str(e)}. Réponse reçue: {response[:200]}..."
                }
            ...
            if "title" not in qcm_data or "questions" not in qcm_data:
                return {
                    "success": False,
                    "error": "Champs 'title' ou 'questions' manquants dans la réponse"
                }
            
            if not isinstance(qcm_data["questions"], list):
                return {
                    "success": False,
                    "error": "Le champ 'questions' doit être une liste"
                }
            
            if len(qcm_data["questions"]) != num_questions:
                return {
                    "success": False,
                    "error": f"Nombre de questions incorrect: {len(qcm_data['questions'])} au lieu de {num_questions}"
                }
            ...
            for i, q_data in enumerate(qcm_data["questions"]):
                if not isinstance(q_data, dict):
                    return {
                        "success": False,
                        "error": f"Question {i+1} n'est pas un objet valide"
                    }
                ...
                for field in required_fields:
                    if field not in q_data:
                        return {
                            "success": False,
                            "error": f"Champ '{field}' manquant dans la question {i+1}"
                        }
                
                if not isinstance(q_data["options"], list) or len(q_data["options"]) != 4:
                    return {
                        "success": False,
                        "error": f"Question {i+1} doit avoir exactement 4 options"
                    }
                
                if not isinstance(q_data["correct_answer"], int) or q_data["correct_answer"] not in [0, 1, 2, 3]:
                    return {
                        "success": False,
                        "error": f"Question {i+1}: correct_answer doit être 0, 1, 2 ou 3"
                    }
